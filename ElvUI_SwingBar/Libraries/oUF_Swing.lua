...
local function SwingStopped(element)
	local element = element.__owner

	if element.Twohand:IsShown() then return end
	if element.Mainhand:IsShown() then return end
	if element.Offhand:IsShown() then return end
...
local function ParryHaste(self, _, _, event, _, _, _, _, tarGUID, _, missType)
	...
	if dualwield then
		...
		elseif percentage > 0.2 then
			...
			element.Twohand.min = now - (element.Twohand.Twohand.max - now) * percentage / (1 - percentage)
			element.Twohand:SetMinMaxValues(element.Twohand.min, element.Twohand.max)
		end
	end
end
